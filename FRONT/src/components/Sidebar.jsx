...
const Sidebar = () => {
  ...
  // Menu items - couleur unifiée (bleu)
  const menuItems = [
  ...
  return (
    <>
      <aside className={`bg-gray-900 text-white flex flex-col h-screen fixed left-0 top-0 transition-all duration-300 z-20 ${isCollapsed ? 'w-20' : 'w-64'}`}>
        {/* Logo */}
        <div className={`p-4 border-b border-gray-800 flex items-center ${isCollapsed ? 'justify-center' : 'justify-between'}`}>
          {!isCollapsed && (
            <div className="flex items-center gap-2">
              ...
              <div>
                <h1 className="text-xl font-bold">CoopTransport</h1>
        ...
        {/* Navigation - Couleur unifiée BLEU */}
              ...
              >
                <div className="relative">
                  <div className="w-8 h-8 rounded-full bg-blue-600 flex items-center justify-center">
                    <span className="font-semibold text-xs">{getUserInitials()}</span>
                  </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-fadeIn">
            ...
            <div className="bg-gradient-to-r from-blue-600 to-indigo-600 p-6 text-white text-center">
              <div className="relative inline-block">
                <div className="w-24 h-24 rounded-full bg-white/20 flex items-center justify-center text-white text-3xl font-bold border-4 border-white">
                  {getUserInitials()}
                </div>
                <div className="absolute bottom-1 right-1 w-5 h-5 bg-green-500 border-2 border-white rounded-full"></div>
              </div>
              <h2 className="text-xl font-bold mt-3">{getFullName()}</h2>
              <p className="text-blue-100 text-sm">{getRoleLabel()}</p>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex items-center gap-3 pb-3 border-b border-gray-100">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-700">{user?.telephone || 'Non renseigné'}</p>
                </div>
              </div>
            ...
            <div className="p-4 border-t border-gray-100 flex gap-3">
              <button 
                ...
                className="flex-1 flex items-center justify-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-all duration-200"
              >
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 transform transition-all animate-fadeIn">
            <div className="text-center">
              ...
              <div className="flex gap-3">
                <button onClick={handleConfirmLogout} className="flex-1 px-4 py-2 bg-red-600 text-white rounded-xl hover:bg-red-700 transition-all">
                  Oui, me déconnecter
                </button>
                <button onClick={handleCancelLogout} className="flex-1 px-4 py-2 bg-gray-200 text-gray-800 rounded-xl hover:bg-gray-300 transition-all">
                  Annuler
                </button>
              </div>
            </div>
